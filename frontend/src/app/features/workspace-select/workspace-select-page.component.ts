...
export class WorkspaceSelectPageComponent {
  ...
  private async bootstrap(): Promise<void> {
    ...
    // Se intenta la entrada al workspace de demostracion SIEMPRE, no solo
    // cuando la lista viene vacia.
    //
    // Probarlo solo sin workspaces dejaba fuera el caso mas comun: quien ya se
    // habia creado uno propio -y vacio- entraba ahi y veia un panel a cero con
    // los datos reales al otro lado. La operacion es idempotente, asi que
    // intentarlo de mas no cuesta nada.
    ...
    const propios = this.workspace.workspaces();
    if (propios.length === 1 && propios[0] !== undefined) {
      this.selectedId.set(propios[0].id);
    }
  }
