...
The repository is not yet a Sentra backend. It is a small Convex TypeScript scaffold with the Agent Component mounted in `convex/convex.config.ts`, one app-owned `threadMetadata` table in `convex/schema.ts`, and support-agent functions in `convex/chat.ts`. Chat public functions have validators but contain comments indicating that authentication and ownership checks are not wired. There are no Sentra tables, HTTP routes, auth configuration, authorization helpers, incident functions, or Convex integration tests. Existing tests cover only pure message normalization (`tests/validation.test.ts`).
